...
class MeasurementsController {
    ...
    def list() {
        ...
        availableMeasurements.each() {
            ...
            measurements << ['name': type,
                             'links': [
                                     measurement: createLink(mapping: 'measurement',
                                             params: [id: type], absolute: true)
                             ]
            ]
        }
    ...
    def show(String id, String filter) {
        ...
        if (errors.size() > 0) {
            return [status: 422, resourceType: 'errors',
                    resource: ['message': 'Validation failed', 'errors': errors]]
        }
    ...
    private static def validateInput(MeasurementTypeName measurementType, MeasurementFilterType filterType) {
        def errors = []
        if (measurementType == null) {
            errors << ['resource': 'measurements',
    ...
    private static def toClientValues(Measurement measurement, TimeFilter timeFilter) {
        switch (measurement.measurementType.name) {
            ...
            case MeasurementTypeName.URINE:
                return ['timestamp': measurement.time,
                        'measurement': measurement.protein.value()]
    ...
    private static def toCgmValues(Measurement measurement, TimeFilter timeFilter) {
        def events = []

        measurement.continuousBloodSugarEvents.each { cgmEvent ->
            if(timeFilter.isLimited && (cgmEvent.time.before(timeFilter.start)||
                    cgmEvent.time.after(timeFilter.end))) {
            ...
            switch (cgmEvent) {
                case ContinuousBloodSugarMeasurement:
                    mapped.measurement = ['value': cgmEvent.glucoseValueInmmolPerl,
                ...
                case CoulometerReadingEvent:
                    ...
                    break
                case InsulinEvent:
                    mapped.measurement = ['value': 15, 'type': 'insulin',
                                          'insulinType': cgmEvent.insulinType.toString(),
                                          'units': cgmEvent.units]
                    events << mapped
                    break
                case ExerciseEvent:
                    mapped.measurement = ['value': 16, 'type': 'exercise',
                                          'exerciseType': cgmEvent.exerciseType.toString(),
                                          'durationInMinutes': cgmEvent.durationInMinutes,
                                          'exerciseIntensity': cgmEvent.exerciseIntensity.toString()]
                    events << mapped
                    break
                case MealEvent:
                    mapped.measurement = ['value': 17, 'type': 'meal',
                                          'carboGrams': cgmEvent.carboGrams,
                                          'foodType': cgmEvent.foodType.toString()]
                    events << mapped
                    break
                case StateOfHealthEvent:
                    mapped.measurement = ['value': 18, 'type': 'state_of_health',
                                          'stateOfHealth': cgmEvent.stateOfHealth.toString()]
                    events << mapped
                    break
                case GenericEvent:
                    mapped.measurement = ['value': 19, 'type': 'generic',
                                          'indicatedEvent': cgmEvent.indicatedEvent]
                    events << mapped
                    break
                default:
